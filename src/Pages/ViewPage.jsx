...
//             if (fetchedCustomer) {
//                 console.log("fetch ", fetchedCustomer)
...
import React, { useState, useEffect } from 'react';
import { useNavigate, useParams } from 'react-router-dom';
import { Box, Skeleton, useTheme, Modal, TextField, Typography, Button } from '@mui/material';
import { useDispatch, useSelector } from 'react-redux';
import { motion } from 'framer-motion';
import { getDetailsById, updateData } from '../Api/Apis';
import PersonalDetails from '../Components/viewPageComps/PersonalDetails';
import LoanDetails from '../Components/viewPageComps/LoanDetails';
import { SwipeableButton } from 'react-swipeable-button';
import CloseIcon from '@mui/icons-material/Close';
...
const ViewPage = () => {
    ...
    const handleLoanUpdate = async (loanIndex) => {
        setIsLoading(true);
        await updateData(customer, dispatch, navigate, id);
        setIsLoading(false);
    };
    ...
    const handleModalOpen = () => {
        setPaidAmount(customer.Loans[0].Amount);
        setIsModalOpen(true);
    };
    ...
    if (!customer) {
        return (
            <Box sx={{ padding: '20px', width: '100%', maxWidth: '1200px', margin: 'auto', backgroundColor: theme.palette.background.default }}>
                <Skeleton variant="rectangular" width="100%" height={200} sx={{ mb: 2, borderRadius: '10px' }} />
                <Skeleton variant="rectangular" width="100%" height={400} sx={{ borderRadius: '10px' }} />
            </Box>
        );
    ...
    return (
        <Box sx={{ padding: '20px', width: '100%', maxWidth: '1200px', margin: 'auto', backgroundColor: theme.palette.background.primary }}>
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
                <PersonalDetails
                    customer={customer}
                    isEdit={isPersonalEdit}
                    setIsEdit={setIsPersonalEdit}
